...
- フロント: Tauri + React + TipTap v3（`/app/src/components/NoteEditor.tsx`）
- ノート保存: ローカルファイルへ Markdown 直接保存（`/app/src/lib/notePersistence.ts`）
- Markdown 変換: `@tiptap/markdown`（エディタ内蔵。`useEditor({ contentType: 'markdown' })` + `editor.getMarkdown()`）
...
画像機能は `CarbonImage` 拡張として `app/src/lib/tiptap/carbon-image-extension/` にモジュール化されている。

- `@tiptap/extension-image` をベースにカスタム拡張 `CarbonImage` を構築
- 拡張内部で `handleDrop` / `handlePaste` を実装し、画像ファイルを検知
- ドロップ直後はローカル `blob:` でプレビュー表示
- 圧縮処理を実行してからアップロードする（`image-compression.ts`）
...
- アップロード/解決は `asset-client.ts` に集約
...
`CarbonImage` 拡張内の `parseMarkdown` / `renderMarkdown` ハンドラで処理する。

- 保存時（TipTap → Markdown）:
  - `carbon://asset/...` を `![alt](carbon://asset/...)` として出力
- 読込時（Markdown → TipTap）:
  - `carbon://asset/...` を抽出
  - `/api/assets/resolve` で短命URLを取得し、画像ノードの `src` 属性を更新

重要:
- 署名URLは保存しない（保存対象は常に `carbon://asset/...`）
- `@tiptap/markdown` がパース/シリアライズを統括し、HTML 中間変換は行わない

### 7.3 API クライアント

画像アップロード/解決の API 呼び出しは `app/src/lib/tiptap/carbon-image-extension/asset-client.ts` に集約されている。
`FormData` 送信時は `Content-Type` を自動設定に委譲する。
...
- 参照切れ検出は `asset_refs` など参照テーブル導入時に段階導入
