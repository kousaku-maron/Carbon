...
import { getImageMimeType } from "../lib/file-kind";
...
export function ImageViewer(props: ImageViewerProps) {
  ...
  useEffect(() => {
    ...
    void readFile(file.path)
      .then((bytes) => {
        if (!alive) return;
        const blob = new Blob([bytes], { type: getImageMimeType(file.path) });
    ...
    <div className="image-viewer">
      ...
      <div
        className={`image-viewer-content ${
          darkBackground ? "image-viewer-content--dark" : "image-viewer-content--light"
        }`}
      >
        <div className="image-viewer-bg-switcher image-viewer-bg-switcher--floating">
          <button
            type="button"
            className={`image-viewer-bg-toggle${darkBackground ? " is-on" : ""}`}
            role="switch"
            aria-checked={darkBackground}
            aria-label="Toggle image background between white and black"
            onClick={() => setDarkBackground((prev) => !prev)}
          >
            <span className="image-viewer-bg-toggle-track">
              <span className="image-viewer-bg-toggle-thumb" />
            </span>
          </button>
          <span className={`image-viewer-bg-switcher-label${darkBackground ? " is-active" : ""}`}>
            Black
          </span>
        </div>
        {hasError ? (
          <p className="image-viewer-error">
            画像を表示できませんでした: <code>{file.name}</code>
          </p>
        ) : loading || !imageSrc ? (
