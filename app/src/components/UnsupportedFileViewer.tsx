...
type UnsupportedFileViewerProps = {
  file: TreeNode;
};

export function UnsupportedFileViewer(props: UnsupportedFileViewerProps) {
  const { file } = props;
  ...
  return (
    <div className="unsupported-file-viewer">
      <header className="unsupported-file-viewer-header">
        <span className="unsupported-file-viewer-title">{file.name}</span>
        <button
          type="button"
      ...
      <div className="unsupported-file-viewer-content">
        <p>
          このファイル形式はまだサポートしていません: <code>{file.name}</code>
        </p>
      </div>
